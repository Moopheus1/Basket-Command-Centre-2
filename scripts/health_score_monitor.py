...
per day (called from fetch_data.py when HEALTH_CHECK_RUN=true - see that
file's main() and .github/workflows/update-data.yml), and watches the
...
Runs once/day on the 4am ET early-ping cron (before the US trading day
starts), NOT on the 16:30 ET heavy EOD pull - the two are unrelated:
fundamentals don't move intraday, so there's no reason to tie this to
yfinance's own once-daily cadence, and checking before the session
opens means the score is fresh by the time anyone's looking at the
dashboard to make a decision, not stale from the prior afternoon.
...
def run(tickers_dict):
    ...
    for sym, entry in tickers_dict.items():
        ...
        try:
            ...
            # A single ticker's data throwing something get_score_with_status
            # didn't anticipate (unexpected type/shape from Equity Lens for
            # that specific company) must never take down the whole EOD run -
            # if it did, docs/health_score_state.json never gets written,
            # which broke the "Commit updated data" step outright the one
            # time this happened. Treat it as a failure for THIS ticker only.
            print(f"[health_score_monitor] {sym}: unexpected error, treating as failure: {e}")
            score, grade, status = None, None, "unexpected_error"
        entry["healthScore"] = score
        entry["healthGrade"] = grade
        statuses[sym] = status
        time.sleep(REQUEST_PAUSE_SECONDS)  # stay polite to an unofficial endpoint
